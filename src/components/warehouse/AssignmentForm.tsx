...
const assignmentSchema = z.object({
  playerId: z.string({
    required_error: "Seleziona un giocatore",
  }),
  baseItemId: z.string({
  ...
  assignDate: z.date({
    required_error: "La data di assegnazione è obbligatoria",
  }),
...
export function AssignmentForm({ items, players, onSubmit, onCancel }: AssignmentFormProps) {
  ...
  useEffect(() => {
    if (selectedItemId) {
      ...
      if (selectedItem) {
        // Only show variants with available quantity
        setSelectedVariants(selectedItem.variants.filter(variant => variant.quantity > 0));
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col">
                <FormLabel>Data restituzione prevista</FormLabel>
                <Popover>
                  <PopoverTrigger asChild>
                    <FormControl>
                      <Button
